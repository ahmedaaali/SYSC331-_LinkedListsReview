...
{//More compact version (very proud of it)
    struct list_node *head = (struct list_node *)malloc(sizeof(struct list_node));
    struct list_node *currC, *curr, *temp;
    for (currC = head, curr = original; curr != NULL; curr = curr->next, currC = currC->next) {
        currC->next = (struct list_node *)malloc(sizeof(struct list_node));
        currC->next->name = (char *)malloc(strlen(curr->name)+1);
        strcpy(currC->next->name, curr->name);
    }
    temp = head; head = head->next; free(temp);
    for (temp = head, currC = head, curr = original; curr != NULL; curr = curr->next, currC = currC->next, temp = head) {
        for (; strcmp(temp->name, curr->contact->name) != 0; temp = temp->next){}
        currC->contact = temp;
    }
    return head;
    /*struct list_node *head = (struct list_node *)malloc(sizeof(struct list_node));
    struct list_node *currC = head;
    struct list_node *curr = original;
    while (curr->next != NULL) {
        currC->name = (char *)malloc(strlen(curr->name)+1);
        strcpy(currC->name,curr->name);
        currC->next = (struct list_node *)malloc(sizeof(struct list_node));
        curr = curr->next;
        currC = currC->next;
    }
